 ...
 *
 */
public class CreatePackedHTML
{
	private static Log log = LogFactory.getLog(CreatePackedHTML.class);
	
	private PackedTrainSet trainSet;
	
	public CreatePackedHTML(PackedTrainSet trainSet)
	{
	...
	public void create() throws Exception
	{
		...
		/** 读取模板文件 */
		String filePath = SystemConfigure.get("TemplateHome") + File.separator + "packed.template";
		...
		/** 生成分析结果 */
		String pathName = SystemConfigure.get("PackedResultHome");
	...
	private String replaceValue(String templateContent)
	{
		...
		String strTemp = "";
		List<PackedResultShow> resultShow = trainSet.getResultShow();
		...
		for (int i = 0; i < resultShow.size(); i++)
		{
			...
			strContentData = replace(strContentData, "{name}", result.getClassName().equals("packed") ? "加壳" : "未加壳");
